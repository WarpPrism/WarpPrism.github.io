...
(function() {
    ...
    var TechPage = {
        ...
        doPaginate: function() {
            ...
            // 计算当前的post数目
            var current_tn_items = [];
        
            for (var j = 0; j < tn_items.length; j++) {
                var post = tn_items[j];
                if ($(post).css("display") != "none") {
                    current_tn_items.push(post);
                }
            }
        
            // 计算分页数，并生成分页按钮
            var oNum = 7; // 定义每页有 7 篇文章
            current_tn_items.slice(7).forEach(function(post) {
                $(post).css("display", "none");
            });
        
            var paginateNum = Math.ceil(current_tn_items.length / oNum);
            ...
            for (i = 0; i < paginateNum; i++) {
                ...
                var closure =  function(i) {
                    item.click(function(e) {
                        ...
                        for (var j = 0; j < current_tn_items.length; j++) {
                            if (j <= range[1] && j >= range[0]) {
                                $(current_tn_items[j]).show();
                            } else {
                                $(current_tn_items[j]).hide();
                            }
                        }
                    });
                    ...
                    item.mouseenter(function() {
                        $(this).css({"box-shadow": "2px 2px 20px 2px #00a1ea"});
                    });
